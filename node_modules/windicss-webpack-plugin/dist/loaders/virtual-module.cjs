...
/* Error: ${error}*/`);
  ...
  if (service.dirty.has("all-modules")) {
    const contents = await Promise.all([...await service.getFiles()].filter((id) => service.isDetectTarget(id)).map(async (id) => [await fs__default.promises.readFile(id, "utf-8"), id]));
