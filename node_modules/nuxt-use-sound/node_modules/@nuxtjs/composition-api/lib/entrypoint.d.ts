...
import { Context, Plugin, Middleware, Module, ServerMiddleware, NuxtConfig, NuxtOptions } from '@nuxt/types';
import vue_router, { Route } from 'vue-router';
...
declare const defineNuxtConfig: (config: NuxtConfig) => Partial<NuxtOptions>;
 ...
 */
declare const wrapProperty: <K extends "$nuxt" | "$data" | "$props" | "$attrs" | "$el" | "$options" | "$parent" | "$root" | "$children" | "$refs" | "$slots" | "$scopedSlots" | "$isServer" | "$ssrContext" | "$vnode" | "$listeners" | "$mount" | "$forceUpdate" | "$destroy" | "$set" | "$delete" | "$watch" | "$on" | "$once" | "$off" | "$emit" | "$nextTick" | "$createElement" | "$meta" | "$router" | "$route" | "$store" | "$config" | "$fetch" | "$fetchState", T extends boolean = true>(property: K, makeComputed?: T | undefined) => () => T extends true ? ComputedRef<CombinedVueInstance<vue, object, object, object, Record<never, any>>[K]> : CombinedVueInstance<vue, object, object, object, Record<never, any>>[K];
/**
 ...
 * @example
  ...
  ```
 */
declare const useRouter: () => vue_router;
/**
